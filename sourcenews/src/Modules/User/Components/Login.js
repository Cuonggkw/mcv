...
class Login extends React.Component {
  ...
  handleCheckReCaptcha = (e) => {
    ...
    grecaptcha.ready(async () => {
      const _token = await grecaptcha
        .execute(process.env.RECAPTCHA_SITE_KEY, { action: "login" })
        .then((token) => token);
  ...
  handleSubmit = (_token) => {
    let _validation = validationForm(
      this.state.values,
      this.state.type == "otp" ? "signInOTP" : "signIn",
      this.state.type
    );
    ...
    if (this.state.status.loading == false && _validation.formIsValid == true) {
      ...
      let _phone = phone(
        this.state.values.phoneCode + this.state.values.phone
      ).phoneNumber;
      ...
      this._isMounted &&
        postApi(process.env.API_USER_URL + "auth/login", _params)
          .then((result) => {
            if (result.data.status == "success") {
              if (this.state.type == "otp") {
                this._isMounted &&
                  this.props.setVerifyOTP("register", { phone: _phone });
                ...
                this.props.renewTimeout = setTimeout(
                  () =>
                    this.props.renewToken(
                      true,
                      result.data.result?.phone ?? result.data.result?.email,
          ...
          .catch((e) => {
            if (e.response) {
              ...
              if (_msg == "Tài khoản của bạn chưa kích hoạt") {
                this._isMounted &&
                  this.props.setVerifyOTP("register", {
                    phone: phone(
                      this.state.values.phoneCode + this.state.values.phone
                    ).phoneNumber,
  ...
  responseGoogle = (response) => {
    if (response && response.accessToken != "") {
      ...
      this._isMounted &&
        postApi(process.env.API_USER_URL + "oauth/login", params)
          .then((result) => {
            if ((result.data.status = "success")) {
              this._isMounted &&
                this.props.setUser({
    ...
    return (
      <React.Fragment>
        <div
          id="modal-signIn"
          className={
            "modal-page animate__animated animate__faster" +
            (_modal.page == "signIn" ? " show animate__fadeIn" : "")
          }
        >
          ...
          <div className="nl-tabs">
            <div className="nl-tabs__inner">
              <div
                className={
                  "tabs-label" + (this.state.type == "otp" ? " active" : "")
                }
                onClick={() => this.handleToggleType("otp")}
              >
                OTP
              </div>
              <div
                className={
                  "tabs-label" +
                  (this.state.type == "password" ? " active" : "")
                }
                onClick={() => this.handleToggleType("password")}
              >
                Password
              </div>
              <div
          ...
          <div className="nl-tabs__panel">
            <form onSubmit={this.handleCheckReCaptcha} autoComplete="off">
              ...
              {this.state.type == "password" && (
                <>
                  <div
                    className={
                      "frm-ctrl" + (_validation.password ? " error" : "")
                    }
                  >
                  ...
                  <a
                    ...
                    onClick={() => this.currentPage("forgotPass")}
                  >
                    Quên mật khẩu
                  </a>
                </>
              ...
              <p
                className={
                  "text-error" + (this.state.msg == "" ? " d-none" : "")
                }
            ...
            <div className="nl-social">
              ...
              <div className="nl-social__list">
                ...
                >
                  <img
                    src="/images/ic-apple.png"
                    title="Đăng nhập với Apple"
                    alt="Đăng nhập với Apple"
                  />
                </IconButton>
              </div>
            </div>
...
const mapDispatchToProps = (dispatch) => {
  ...
  return {
    ...
    setVerifyOTP: (type, val) => {
      dispatch(_action.setVerifyOTP(type, val));
    },
  };
};
