...
class Common {
    ...
    _page[page].map((k) => {
      ...
      if (k == "videoFileInfo") {
        if (typeof obj[k] == "undefined") {
          _result.formIsValid = false;
          _result.errors[k] =
            (typeof _pageTitle[page] !== "undefined" &&
            typeof _pageTitle[page][k] !== "undefined"
              ? _pageTitle[page][k]
              : k) + " không được bỏ trống";
          _result.msg += "Vui lòng upload video<br/>";
        } else {
          if (
            obj[k].size / 1000 >= 100000 &&
            obj[k].type.indexOf("video/mp4") >= 0
          ) {
            _result.formIsValid = false;
      ...
      if (k == "phone" && obj && obj[k] && obj[k] != "") {
        if (obj[k].indexOf("+") != -1) {
          ...
          let validate = phone(phone(obj.phoneCode + obj[k]).phoneNumber, {
            country: "",
            validateMobilePrefix: true,
          });
          if (validate.isValid == false) {
          ...
          _result.formIsValid = false;
          _result.errors[k] =
            "Số điện thoại cần bắt đầu bằng mã vùng (Ví dụ: +84*********)";
          _result.msg +=
            "Số điện thoại cần bắt đầu bằng mã vùng (Ví dụ: +84*********)<br/>";
        }
      }
      if (
        (page == "signUp" ||
          page == "changePass" ||
          page == "profile" ||
          page == "changeProfilePass" ||
          page == "updateProfilePass") &&
        k == "password" &&
        obj &&
        obj[k] &&
        obj[k] != ""
        ...
        if (!_regex.test(obj[k])) {
          ...
          _result.msg +=
            "Mật khẩu có ít nhất 8 ký tự: 1 chữ hoa, 1 số và 1 ký tự đặc biệt<br/>";
        }
      }
      if (
        (page == "signUp" ||
  ...
  getThumb = async (videoId) => {
    let _img = await axios
      .get(
        "https://api.dailymotion.com/video/" +
          videoId +
          "?fields=thumbnail_720_url"
      )
      .then((resp) => resp.data.thumbnail_720_url)
      .catch((e) => "");
    return _img;
  };
  ...
  detectMobile = (userAgent) => {
    ...
    if (Boolean(userAgent.match(/zalo/i)) == true)
      _result.referer = "https://chat.zalo.me";
    if (Boolean(userAgent.match(/fban|fbdv/i)) == true)
      _result.referer = "https://www.messenger.com";
  ...
  getMsg = (arr) => {
    ...
    if (
      arr?.data &&
      arr?.data?.errors &&
      typeof arr.data.errors.msg !== "undefined" &&
      typeof arr.data.errors.msg.response !== "undefined"
    ) {
      ...
      arr?.data &&
      arr?.data?.errors &&
      typeof arr.data.errors[0] !== "undefined" &&
      typeof arr.data.errors[0].msg !== "undefined"
    ) {
